...
class _TarotReadingScreenState extends State<TarotReadingScreen> with TickerProviderStateMixin {
  ...
  // Card Picking State
  bool _isFanSpread = true; // ?àÏù¥?ÑÏõÉ ?†Í?
  bool _showLightning = false; // Î≤àÍ∞ú ?¥Ìéô???†Í?
  final int _totalCards = 78; // ?ÑÏ≤¥ 78??
  ...
  void initState() {
    ...
    _lightningAnimController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 100), // Îß§Ïö∞ Îπ†Î•¥Í≤?Î≤àÏ©ç??
    );
  ...
  void _generateAiReading() async {
    ...
    String prompt = "?¨Ïö©?êÍ? ÏßàÎ¨∏ ?ÜÏù¥ $spreadName Î∞∞Ïó¥Î≤ïÏúºÎ°??ÄÎ°úÏπ¥?úÎ? ÎΩëÏïò?µÎãà?? ÎΩëÌûå Ïπ¥Îìú?§ÏùÑ Î∞∞Ïó¥Î≤ïÏùò Í∞??ÑÏπò??ÎßûÍ≤å ?¥ÏÑù?òÍ≥† ?ÑÎ∞ò?ÅÏù∏ ?¥ÏÑ∏?Ä Ï°∞Ïñ∏???¥Ï£º?∏Ïöî. ?µÎ??Ä Î∞òÎìú???πÏã†???òÎ•¥?åÎÇò ?§ÏúºÎ°?Î∞îÎ°ú ?úÏûë?¥Ï£º?∏Ïöî.";
  ...
  Widget _buildPickingView() {
    return Stack(
      children: [
        SafeArea(
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              // ?ÅÎã® Îπ??¨Î°Ø???úÏûë?òÎäî ?ïÌôï??Y Ï¢åÌëú
              // Padding top 80 + ?ÅÎã® ?çÏä§?∏Row ?Ä??30 + SizedBox 20 = 130
              final double slotsTopY = 130.0; 

              return Stack(
                clipBehavior: Clip.none,
                children: [
              // 1. Î∞∞Í≤Ω Î∞?UI ?îÏÜå ?àÏù¥??
              Column(
                children: [
                  Padding(
                  ...
                  // Îπ??¨Î°Ø?Ä ?úÏãú?òÏ? ?äÍ≥† Ïπ¥Îìú Í≤πÏπ® ?àÏù¥?ÑÏõÉ???¨Ïö©
                  SizedBox(height: slotHeight),
                ],
              ),
              
              // 2. ?∏ÌÑ∞?ôÌã∞Î∏?Ïπ¥Îìú ?àÏù¥??(?ÑÏ≤¥ ?îÎ©¥ Í∏∞Ï? ?àÎ? Ï¢åÌëúÎ°??†ÎãàÎ©îÏù¥??
        ...
        // 3. Î≤àÍ∞ú ?¥Ìéô???àÏù¥??(Ïπ¥ÎìúÍ∞Ä Î™®Îëê ?†ÌÉù????3Ï¥àÍ∞Ñ ?úÏãú)
        // ?îÎ©¥ ?ÑÏ≤¥Î•???èÑÎ°?SafeArea ?∏Î???Î∞∞Ïπò
        if (_showLightning) ...[
          // ?îÎ©¥ ?ÑÏ≤¥Í∞Ä ?¥Îëê?åÏ????®Í≥º (Í≤Ä?Ä Î∞∞Í≤Ω Îß§Ïπ≠)
          Positioned.fill(
          ...
          // ?òÏ†ïÍµ?Î≤àÍ∞ú) ?¥Î?ÏßÄ (?¨Í∏∞ Î∞òÏúºÎ°?Ï∂ïÏÜå)
          Positioned.fill(
            child: Center(
                  ...
                  children: [
                    ...
                    if (_currentDiary != null)
                      OutlinedButton.icon(
                        onPressed: () async {
                          if (_currentDiary != null) {
                            await TarotDiaryService.instance.saveToLocalOnly(_currentDiary!);
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
  ...
  void _shareReadingResult() async {
    try {
      ...
      if (kIsWeb) {
        // ?πÏóê?úÎäî ?åÏùº ?úÏä§???ëÍ∑º??Î∂àÍ??•ÌïòÎØÄÎ°??çÏä§?∏Îßå Í≥µÏú†
