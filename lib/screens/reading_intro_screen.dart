...
class _ReadingIntroScreenState extends State<ReadingIntroScreen> with SingleTickerProviderStateMixin {
  ...
  void _playGreeting() {
    _ttsService.stop();
    // TTS ?åÏÑ± ?¨ÏÉù ?úÍ±∞ ?îÏ≤≠???∞Îùº Ï£ºÏÑù Ï≤òÎ¶¨
    // final greeting = '?¥Îñ§ ?¥Î™Ö???§Ïó¨?§Î≥ºÍπåÏöî? ?πÏã†??ÎØ∏ÎûòÍ∞Ä Í∂ÅÍ∏à?òÍµ∞??';
    ...
    return Scaffold(
      body: Stack(
        ...
        children: [
          ...
          // 2. ÎßàÎ?(Ï§ëÏïô)Î•??úÏô∏?òÍ≥† Î∞∞Í≤Ω Ï™ΩÏóêÎß?Î≥¥Îùº??Î∂àÎπõ???®Ïâ¨??ÎπõÎÇò???®Í≥º
          AnimatedBuilder(
              ...
              return Container(
                decoration: BoxDecoration(
                  gradient: RadialGradient(
                    center: const Alignment(0, -0.2), // ÎßàÎ????ÅÎã® Ï§ëÏã¨ ?ÑÏπò
                    radius: 1.2,
                    colors: [
                      Colors.transparent, // Ï§ëÏïô(ÎßàÎ?)?Ä ?¨Î™Ö?òÍ≤å ?†Ï?
                      Colors.purple.withOpacity(_glowAnimation.value), // Ï£ºÎ??Ä Î≥¥Îùº??Îπ?                      Colors.deepPurple.withOpacity(_glowAnimation.value * 0.8),
                    ],
                    stops: const [0.35, 0.8, 1.0], // 0.35ÍπåÏ????¨Î™Ö?¥ÏÑú ÎßàÎ?Í∞Ä Î≥¥ÏûÑ
                  ),
                ),
              );
            },
          ...
          // 3. ?çÏä§??Í∞Ä?ÖÏÑ±???ÑÌïú ?ÑÏ£º ?ÖÏ? ?¥Îëê???§Î≤Ñ?àÏù¥
          Container(
