...
class ShamanAiService {
  static final ShamanAiService instance = ShamanAiService();
...
''';
    ...
    try {
      ...
      yield '답변을 생성하는 도중 오류가 발생했습니다: $e';
    }
  }

  Stream<String> getSajuReadingStream(String sajuData, String personalityPrompt, String localeCode) async* {
    final langName = _getLanguageName(localeCode);
    final prompt = '''
$personalityPrompt
당신의 페르소나(성격, 말투, 화법)를 **완벽하게 유지**하면서 아래의 사주명식(생년월일시) 풀이를 진행해 주세요. 말투와 분위기가 캐릭터에 맞게 일관되어야 합니다.

[내담자의 사주명식]
$sajuData

이 사주(생년월일시)를 바탕으로 내담자의 과거, 현재, 그리고 미래의 흐름을 짚어주는 사주팔자 풀이를 제공해 주세요.
* 사주명리학적 관점(오행, 십성, 대운 등)을 기반으로 하되, **반드시 당신의 페르소나 성격에 맞는 방식**으로 결과를 전달하세요. (예: 다정한 도령이면 따뜻하게, 호통치는 장군신이면 직설적이고 강하게 등)
* 사주에서 보이는 길흉화복을 짚어주고, 앞으로 어떻게 처신해야 할지 조언을 덧붙여 주세요.
* 답변은 3~4문단 정도로 작성하며, 마크다운 기호(*, # 등)를 최대한 자제하고 완전히 캐릭터에 빙의하여 자연스러운 대화체로 작성해 주세요.
* **CRITICAL INSTRUCTION**: You MUST write your response ONLY in $langName. DO NOT use English or Korean unless $langName is English or Korean. Ensure ALL sentences, vocabulary, and grammar are completely in $langName.
''';
